/* screen tables */
function DoNew(arg, indexfile) {
	document.location = indexfile + "?mode=new&type=" + arg;
};

function DoDeleteDomains() {
	if (!CheckSelection()) {
		alert('No domain(s) selected.');
		return false;
	};
	if (confirm('Are you sure to delete these domains?')) {
		var form = $("table_form");
		if (form) form.submit();
	};
}
...
function DoDisableUsers() {
	...
	var form = $("table_form");
	var action = $("action");
	if (action) action.value = "disable";
	if (form) form.submit();
};
...
function CheckSelection() {
	if (Selection && Selection.CheckCount() == 0) return false;
	return true;
}

var switch_prefix = "switcher_";
var switchItems = new Array();

function SwitchAdd(id) {
	switchItems.push(switch_prefix + id);
};

function SwitchInit(name) {
	SwitchItem(switchItems, switch_prefix + $(name).value);
};

function SwitchMain(obj) {
	SwitchItem(switchItems, switch_prefix + obj.value);
};

/* pageswitcher classic */
function CPageSwitcher() {
	this._skinName = _apPath;
	this._mainCont = null;
	...
	Show: function (page, perPage, count, beginOnclick, endOnclick) {
		...
		if (count > perPage) {
			...
			if (firstPage != lastPage) {
				if (firstPage > 1) {
					strPages += '<a href="javascript:void(0);" onclick="' + beginOnclick + '1' + endOnclick + ' return false;"><img title="First Page" style="width: 8px; height: 9px;" src="' + this._skinName + '/images/page_switchers/inbox_first_page.gif" /></a>';
					strPages += '<a href="javascript:void(0);" onclick="' + beginOnclick + firstPage + endOnclick + ' return false;"><img title="Previous Page" style="width: 5px; height: 9px;" src="' + this._skinName + '/images/page_switchers/inbox_prev_page.gif" /></a>';
				};
				...
				if (pagesCount > lastPage) {
					strPages += '<a href="javascript:void(0);" onclick="' + beginOnclick + lastPage + endOnclick + ' return false;"><img title="Next Page" style="width: 5px; height: 9px;" src="' + this._skinName + '/images/page_switchers/inbox_next_page.gif" /></a>';
					strPages += '<a href="javascript:void(0);" onclick="' + beginOnclick + pagesCount + endOnclick + ' return false;"><img title="Last Page" style="width: 8px; height: 9px;" src="' + this._skinName + '/images/page_switchers/inbox_last_page.gif" /></a>';
				};
...
CList.prototype = {
	ResizeBody: function(mode) {
		if (!Browser.IE || (Browser.IE && Browser.Version >= 7)) {
			...
			var tableHeight = this._contactListTbl.offsetHeight	+ this._contactListTop.offsetHeight;
			var cardHeight = 0;
			...
			if (height < biggerH) height = biggerH;
	...
	GetExternalHeight: function(){
		var res = 0;
		var offsetHeight = this._logo.offsetHeight;		if (offsetHeight || offsetHeight === 0) { res += offsetHeight; }
		offsetHeight = this._accountsBar.offsetHeight;	if (offsetHeight || offsetHeight === 0) { res += offsetHeight; } else { return this._externalHeight; }
