...
<script type="text/javascript">
	...
	useredittabs.push("content_custom_tab_3");
	useredittabs.push("content_custom_tab_4");

	function SelectAllOnSubmit() {
		SelectListAll("AliasesListDDL");
		SelectListAll("ForwardsListDDL");
	}
	...
	function SetAllOf()
	{
		...
		$('custom_tab_3').className = 'wm_settings_switcher_item';
		$('custom_tab_4').className = 'wm_settings_switcher_item';
	}
</script>
...
<div class="wm_settings_accounts_info" style="width: 100%; margin: 15px 0px">
	...
	<div id="custom_tab_4" class="wm_settings_switcher_item" onclick="SwitchItem(useredittabs, 'content_custom_tab_4'); SetAllOf(); this.className = 'wm_settings_switcher_select_item';">
		<a href="javascript:void(0)">Forwards Settings</a>
	</div>
	<div id="custom_tab_3" class="wm_settings_switcher_item" onclick="SwitchItem(useredittabs, 'content_custom_tab_3'); SetAllOf(); this.className = 'wm_settings_switcher_select_item';">
		<a href="javascript:void(0)">Alias Settings</a>
	</div>
	...
	<table class="wm_contacts_view_new">
		...
		<tr>
			...
			<td align="left" colspan="2">
				<span class="<?php $this->data->PrintInputValue('classLoginSpan'); ?>"><?php $this->data->PrintInputValue('txtIncomingLogin'); ?></span>
				<input name="txtIncomingLogin" type="text" id="txtIncomingLogin" class="<?php $this->data->PrintInputValue('classLoginField'); ?>" style="width: 350px" maxlength="100" value="<?php $this->data->PrintInputValue('txtIncomingLogin'); ?>" />
			</td>
		</tr>
		<tr>
			<td align="right">
		...
		<tr class="<?php $this->data->PrintInputValue('classHideEmailTr'); ?>">
			<td align="right">
				<nobr>* Email:</nobr>
			</td>
			<td align="left" colspan="2">
				<span><?php $this->data->PrintValue('txtEmail'); ?></span>
				<input name="txtEmail" type="text" id="txtEmail" class="wm_hide" style="width: 350px" maxlength="100" value="<?php $this->data->PrintInputValue('txtEmail'); ?>" />
			</td>
		</tr>
		<tr>
			<td align="right">
...
<div id="content_custom_tab_3" style="padding-top: 10px;">
<table class="wm_contacts_view_new">
<?php
	$path = $this->data->GetIncludeConrolPath('aliases-control');
	if (false !== $path)
	{
		include $path;
	}
...
<div id="content_custom_tab_4" style="padding-top: 10px;">
<table class="wm_contacts_view_new">
<?php
	$path = $this->data->GetIncludeConrolPath('forwards-control');
	if (false !== $path)
	{
		include $path;
	}
...
<?php $this->data->PrintValue('StartWmJs'); ?>
	...
	Validator.RegisterAllowUserLoginSymbols($('AccountAliasID'));
	Validator.RegisterAllowEmailSymbols($('AccountForwardID'));

	$("AliasesListDDL").form.onsubmit = function() {
		OnlineLoadInfo(_langSaving);
		SelectAllOnSubmit();
	};

	$("AccountAliasID").onkeypress = function(ev) {
		ev = (window.event) ? window.event : ev;
		if (ev.keyCode == 13) {
			AddValueToList($('AccountAliasID'), $('AliasesListDDL'));
			return false;
		}
		return true;
	};

	$("AliasesListDDL").onkeypress = function(ev) {
		ev = (window.event) ? window.event : ev;
		if (ev.keyCode == 46) {
			DeleteSelectedFromList($('AliasesListDDL'));
		}
		return (ev.keyCode != 13);
	};

	$("AccountForwardID").onkeypress = function(ev) {
		ev = (window.event) ? window.event : ev;
		if (ev.keyCode == 13) {
			AddForwardEmailToList();
			return false;
		}
		return true;
	};

	function AddForwardEmailToList() {
		var inp = $('AccountForwardID');
		if (IsEmailAddress(inp)) {
			AddValueToList(inp, $('ForwardsListDDL'));
		}
	}

	$("ForwardsListDDL").onkeypress = function(ev) {
		ev = (window.event) ? window.event : ev;
		if (ev.keyCode == 46) {
			DeleteSelectedFromList($('ForwardsListDDL'));
		}
		return (ev.keyCode != 13);
	};
