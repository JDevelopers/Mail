...
var allTimeZone = "" +
"<select id='defTimeZone' style='width: 300px;' name='defTimeZone' class='wm_input'>" +
...
function ASSettings() {
	...
	this.settings_frm	= document.getElementById('cal_form');
...
ASSettings.prototype = {
	...
	InitLang: function(key, value)
	{
		this.Langs[key] = value;
	},
	
	TakePhrase: function(key)
	{
		return (this.Langs[key]) ? this.Langs[key] : key;
	},

	Init: function (set_workdayStarts, set_workdayEnds, set_timeFormat, defaultTimeZone) {
	
		this.set_DefaultTimeZone = defaultTimeZone;
		...
		this.LoadTimeZones((this.allTimeZones.value == 1), this.set_DefaultTimeZone);
	...
	CheckWorkdayTime: function() {
		...
		if (parseInt(this.WorkdayStarts.value) >= parseInt(this.WorkdayEnds.value)) {
			wrongInterval = true;
			this.ShowError(this.TakePhrase("CheckWorkdayTimeError"))
		}
		else {
			wrongInterval = false;
		}
	...
	{
        var obj = this;
        var _defTimeZone = "<select id='defTimeZone' style='width: 300px;' name='defTimeZone' class='wm_input'>";
        var i = "";
