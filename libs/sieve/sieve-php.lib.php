...
class sieve {
  ...
  function WriteLog($sLogDesc)
  {
		if ($this->Log)
		{
			$this->Log->WriteLine('SIEVE: '.$sLogDesc);
		}
  }

  function __fgets($iLen = 1024)
  {
	  $sLine = fgets($this->fp, 1024);
  ...
  function get_response()
  {
    ...
    if($this->token[0] == "NO"){
        ...
        if($this->mtoken[0] == "{"){
            ...
            $this->line = $this->__fgets($this->err_len);
            ...
            while($this->err_recv < $this->err_len){
                ...
                $this->line = $this->__fgets($this->err_len-$this->err_recv);
                $this->error_raw[]=substr($this->line, 0, strlen($this->line) -2);    //we want to be nice and strip crlf's
                $this->err_recv += strlen($this->line);
            } /* end while */
            $this->line = $this->__fgets(1024);	//we need to grab the last crlf, i think.  this may be a bug...
  ...
  function sieve_login() {
	...
	$this->WriteLog('start connect to '.$this->host.':'.$this->port);
  ...
  function sieve_sendscript($scriptname, $script) {
	...
	$this->__fputs('PUTSCRIPT "'.$scriptname.'" {'.$len.'+}');
	$this->__fputs($this->script);
  ...
  function sieve_havespace($scriptname, $scriptsize)   {
    if($this->loggedin==false)
        return false;
	$this->__fputs('HAVESPACE "'.$scriptname.'" '.$scriptsize);
    return sieve::get_response();
  }  
  ...
  function sieve_setactivescript($scriptname)   {
	...
	$this->__fputs('SETACTIVE "'.$scriptname.'"');
    return sieve::get_response();
  ...
  function sieve_getscript($scriptname) {
	...
	$this->__fputs('GETSCRIPT "'.$scriptname.'"');
    return sieve::get_response();
  }
  ...
  function sieve_deletescript($scriptname)   {
	...
	$this->__fputs('DELETESCRIPT "'.$scriptname.'"');
    return sieve::get_response();
  }
  ...
  function authenticate() {
    ...
    switch ($this->auth_in_use) {
        ...
        case "PLAIN":
			...
			$this->__fputs('AUTHENTICATE "PLAIN" {'.$this->len.'+}');
			$this->__fputs($auth);
	...
	case "LOGIN":
		 ...
		 $this->__fputs('AUTHENTICATE "LOGIN"');
		 $this->__fputs('{'.strlen($login).'+}');
		 $this->__fputs($login);
		 $this->__fputs('{'.strlen($pass).'+}');
		 $this->__fputs($pass);
