 ...
 */
	...
	class WebMailMessage extends MailMessage
	{
		...
		{
			$Body = $this->HasPlainText()
				? $this->TextBodies->PlainTextBodyPart
				: $this->TextBodies->HtmlToPlain();
	
			$Body = str_replace("\r", '', $Body);
			$ReplaceStrings = array();
			$Pattern = '/(http|https|ftp|telnet|gopher|news|file|wais|svn):\/\/([a-z0-9+-=%&@:_\.~?]+[#a-z0-9+]*[^\.<>()\s"\'])/ie';
			$Replace = "GetStrReplacement('<a href=\"\\1://\\2\" target=\"_blank\">\\1://\\2</a>', \$ReplaceStrings)";
			$Body = preg_replace($Pattern, $Replace, $Body);
			
			$Pattern = '/(www\.[a-z0-9+-=%&@:_\.~?]+[#a-z0-9+]*[^\.<>()\s"\'])/ie';
			$Replace = "GetStrReplacement('<a href=\"http://\\1\" target=\"_blank\">\\1</a>', \$ReplaceStrings)";
			$Body = preg_replace($Pattern, $Replace, $Body);
			
			/**/
			$Pattern = '/ ([a-z0-9-\.]+\.(com|net|org)[a-z0-9+-=%&@:_\.~?]*[#a-z0-9+]*[^\.<>()\s"\'])/ie';
			$Replace = "GetStrReplacement(' <a href=\"http://\\1\" target=\"_blank\">\\1</a>', \$ReplaceStrings)";
			$Body = preg_replace($Pattern, $Replace, $Body);
			/**/
			...
			$Body = str_replace('  ', '&nbsp;&nbsp;', $Body).$this->_addTrimMesage(0);
